...
# SERVICE_URL = os.environ.get('NEO4J_HOST', 'localhost')

SERVICE_URL = os.getenv('NEO4J_URI', 'bolt://neo4j:7687')
    ...
    if FLASK_FIRST_LAUNCH is True:
        ...
        if BACKGROUND_CHECK is True:
            ...
            try:
                ...
                NEO4J_REQUEST = Graph(SERVICE_URL, auth=(USERNAME, PASSWORD))
                def neo_query_1():
                    global NEO4J_REQUEST
            ...
            for key, value in os.environ.items():
                if ("NEO4J_CORE" in key or "NEO4J_REPLICA" in key) and "PORT_7687_TCP_ADDR" in key and not "ADMIN" in key:
                    ...
                    try:
                        NEO4J_REQUEST = Graph('bolt://' + value + ':7687', auth=(USERNAME, PASSWORD))
                        def neo_query_2():
                            global NEO4J_REQUEST
                            f_file = open('/tmp/result', 'wb')
                            f_file.close()
...
if __name__ == "__main__":
    port = int(os.getenv("NEO4J_METRICS_PORT"))
    host = str(os.getenv("NEO4J_METRICS_HOST"))
...
#     os.environ["GUNICORN_CMD_ARGS"] = f"-w 1 -b 0.0.0.0:{port}"
#     run()
